...
```go
func (*BindingError) Unwrap() error
```
...
BindJSONOrError binds JSON body and returns a Problem on failure. Returns nil if binding succeeds, or a BadRequest Problem if it fails. This is a convenience method for the common pattern of returning an error response on bind failure.

```go
func (*Ctx) BindJSONOrError(v any) error
```
...
| Field | Type | Description |
| ----- | ---- | ----------- |
| Addr | `string` | Addr is the address the server listens on. Default is ":8080". |
| ReadTimeout | `time.Duration` | ReadTimeout is the maximum duration for reading the entire request. Default is 30 seconds. |
| WriteTimeout | `time.Duration` | WriteTimeout is the maximum duration before timing out writes of the response. Default is 30 seconds. |
| IdleTimeout | `time.Duration` | IdleTimeout is the maximum amount of time to wait for the next request when keep-alives are enabled. Default is 120 seconds. |
| GracePeriod | `time.Duration` | GracePeriod is the maximum duration to wait for active connections to finish during graceful shutdown. Default is 30 seconds. |
| TLSCertFile | `string` | TLSCertFile is the path to the TLS certificate file. If set along with TLSKeyFile, the server will use TLS. |
| TLSKeyFile | `string` | TLSKeyFile is the path to the TLS key file. If set along with TLSCertFile, the server will use TLS. |
| TLSConfig | `*tls.Config` | TLSConfig is a custom TLS configuration for the server. If set, TLSCertFile and TLSKeyFile are ignored. |
| MaxHeaderBytes | `int` | MaxHeaderBytes is the maximum size of request headers. Default is 0 (no limit). |
| HideBanner | `bool` | HideBanner hides the banner on startup. Default is false. |
| Banner | `string` | Banner is a custom banner to display on startup. If set, HideBanner is ignored. |
| ErrorHandler | `ErrorHandler` | ErrorHandler is a custom error handler for the server. If not set, the default error handling (RFC 7807 Problem Details) is used. |
| BasePath | `string` | BasePath is a base path prefix for all routes. All registered routes will be prefixed with this path. For example, with base path "/api/v1", a route "/users" becomes "/api/v1/users". The base path should start with "/" but should not end with "/" (it will be normalized). |
| AutoPort | `bool` | AutoPort enables automatic port selection when the configured port is in use. When enabled, the server will try incrementing ports until it finds an available one. This is primarily useful for development environments. Default is false. |
| MaxPortAttempts | `int` | MaxPortAttempts is the maximum number of ports to try when AutoPort is enabled. Default is 10. |
| LogOutput | `middleware.LogOutputFunc` | LogOutput is a callback that receives request log data for output. Use middleware.TextOutput() for Morgan.js-style formatted logs. Use middleware.TextOutputWithOptions() for custom formatting. If not set, defaults to dev format text output. |
...
Default creates a new Server with sensible defaults for development. It includes RequestID, Logger (using logs package), and Recover middleware. If opts is nil, default options will be used.
...
New creates a new Server with the provided options. If opts is nil, default options will be used.
...
Start starts the server and blocks until shutdown. If an address is provided, it will override the Addr option. If the address is not provided and the Addr option is not set, it will use ":8080". This is a convenience method that calls Run with a background context.
...
```go
// Example usage of VersionHandlerMap
var value VersionHandlerMap
// Initialize with appropriate value
```
...
```go
type VersionHandlerMap map[string]*ast.IndexListExpr
```
...
### BindAll
BindAll binds path parameters, query parameters, headers, and JSON body to a struct. Uses struct tags to determine binding sources: - `path:"name"` - binds from URL path parameters - `query:"name"` - binds from URL query parameters - `header:"name"` - binds from HTTP headers - `json:"name"` - binds from JSON body - `form:"name"` - binds from form data

```go
func BindAll(c *Ctx) (T, error)
```
...
```go
// Example usage of BindAll
result := BindAll(/* parameters */)
```

### BindAllValidate
BindAllValidate binds and validates the request. Calls Validate() if the type implements Validatable.

```go
func BindAllValidate(c *Ctx) (T, error)
```
...
```go
// Example usage of BindAllValidate
result := BindAllValidate(/* parameters */)
```
...
### BindHeaderTo
BindHeaderTo binds HTTP headers to a struct. Uses the `header` struct tag to determine field names.

```go
func BindHeaderTo(c *Ctx) (T, error)
```
...
```go
// Example usage of BindHeaderTo
result := BindHeaderTo(/* parameters */)
```
...
```go
// Example usage of BindJSONTo
result := BindJSONTo(/* parameters */)
```
...
### BindPathTo
BindPathTo binds URL path parameters to a struct. Uses the `path` struct tag to determine field names.

```go
func BindPathTo(c *Ctx) (T, error)
```
...
```go
// Example usage of BindPathTo
result := BindPathTo(/* parameters */)
```
...
### BindQueryTo
BindQueryTo binds URL query parameters to a struct. Uses the `query` struct tag to determine field names.

```go
func BindQueryTo(c *Ctx) (T, error)
```
...
```go
// Example usage of BindQueryTo
result := BindQueryTo(/* parameters */)
```
...
### HandleVersions
HandleVersions wraps a generic Handler into an http.HandlerFunc that returns a versioned response. The version is determined by the header value specified by headerName. If no version header is present, defaultVersion is used.

```go
func HandleVersions(headerName, defaultVersion string, versions *ast.IndexListExpr) http.HandlerFunc
```
...
```go
// Example usage of HandleVersions
result := HandleVersions(/* parameters */)
```
